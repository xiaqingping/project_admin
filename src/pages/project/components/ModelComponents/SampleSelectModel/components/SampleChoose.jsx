...
class SampleChoose extends React.Component {
  ...
  handleDataFormat = () => {
    ...
    list.forEach(item => {
      const fileCheckedLen = item.sampleProperties.filter(v => v.isChoose)
        .length;
  ...
  handleCheckboxChange = (e, row, item, index) => {
    ...
    if (item) {
      ...
      const checkedLen = selectRow.sampleProperties.filter(r => r.isChoose)
        .length;
  ...
  handleOk = () => {
    ...
    if (!id) {
      // 点击选择样品过来的。
      const checkedSample = tableData.filter(
        item => item.indeterminate || item.isChoose,
      );
    ...
    const columns = [
      {
        ...
        render: (text, record, index) => (
          <div
            className={
              disabled ? 'project_manage_params_sample_choose_checkbox' : ''
            }
          >
            <Checkbox
              onChange={e =>
                this.handleCheckboxChange(e, record, undefined, index)
              }
            ...
            return (
              <div
                ...
                className={classnames(
                  'project_manage_params_sample_choose_checkbox_style',
                  {
              ...
              >
                <div title={item.sequenceFileName}>
                  <Checkbox
                    onChange={e =>
                      this.handleCheckboxChange(e, record, item, index)
                    }
          ...
          <div>
            {row.sampleProperties &&
              ...
              row.sampleProperties.map(item => {
                return (
                  <div
                    className="project_manage_params_sample_choose_checkbox_style"
                    key={item.id}
                  >
