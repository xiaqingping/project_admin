...
class SampleGroup extends React.Component {
  ...
  selectUpdateGroup = (fromDidMount, datas, colus) => {
    ...
    selList.map(item => {
      ...
      newRow = {
        sampleId: item.id,
        sample: item.sampleAlias || item.sampleName,
      ...
      if (mapGro[item.id]) {
        ...
        // 添加这行就可以解决上面的问题
        newRow.sample = item.sampleAlias || item.sampleName;
        newRow.sampleName = item.sampleName;
  ...
  getRowDataGroup = (groupList, sampleList, groupColumns) => {
    ...
    groupList.forEach(groupItem => {
      // 分组方案下的 样品列表 为空
      if (
        groupItem.samples === null ||
        (groupItem.samples && !groupItem.samples.length)
      ) {
        groupItem.groups.forEach(groItem => {
          groItem.samples.forEach(samItem => {
      ...
      // 分组方案下的 分组列表 为空
      if (
        groupItem.groups === null ||
        (groupItem.groups && !groupItem.groups.length)
      ) {
        groupItem.samples.forEach(samItem => {
          samples.push(samItem);
        });
    ...
    sampleList.forEach(item => {
      newSample.forEach(it => {
        if (item.id === it.sampleId) {
          ...
          newIt.sample = it.sampleAlias || it.sampleName;
  ...
  handleTitleBlur = (e, item, index, id) => {
    ...
    const cols = [...columns];
    const columns1 = JSON.parse(JSON.stringify(columns));
    const dupTitles = this.getDupTitles(cols);
    let valid = true;
    ...
    if (dupTitles.includes(value)) {
      if (item && item.dupTitle !== value) {
        valid = false;
        message.warning('存在相同表头字段');
      }
      if (!item) {
        columns.forEach(it => {
          if (it.id === id) {
            if (it.dupTitle !== value) {
              valid = false;
              message.warning('存在相同表头字段');
            }
          }
  ...
  handleAdd = () => {
    ...
    if (cols.length > 1) {
      cols.forEach(item => {
        if (item.dupTitle && item.dupTitle.indexOf('分组方案_') !== -1) {
          const endNum = item.dupTitle.split('_')[1];
    ...
    if (NumberList.includes(max + 1)) {
      titleMax = Math.max.apply(null, NumberList);
    }
  ...
  selectRender = (value, row, index, color1, col) => {
    ...
    return (
      <AutoComplete
        style={{ width: '70%' }}
        onBlur={e =>
          this.handleGroupSelectBlur(e, value, row, index, col, color1)
        }
        defaultValue={value}
        disabled={disabled}
      >
  ...
  verifyData = (groupSchemeData, columns) => {
    ...
    if (
      (isRequired === 'true' && num === 2) ||
      (isRequired === 'true' && !datas.length)
    ) {
      this.errMessage = '分组方案为必须项，请设置分组方案！';
      this.validPass = false;
      return false;
    }
    if (!datas.length && num > 2) {
      this.errMessage = '存在空的分组方案';
      this.validPass = false;
      return false;
    }
    for (let i = 2; i < num; i++) {
  ...
  formatSubmitData = (groupSchemeData, columns) => {
    // conformapst { groupSchemeData, columns } = this.state;
    // 数据整理
    let tableHeard = [];
    columns.forEach((item, index) => {
      if (index !== 0 && typeof item.id === 'number') {
        tableHeard = [
          ...tableHeard,
          { groupSchemeName: item.dupTitle, samples: [], groups: [] },
        ];
      }
    });

    tableHeard.forEach(item => {
      columns.forEach(col => {
        if (col.dupTitle === item.groupSchemeName) {
          const header = col.dataIndex;
          groupSchemeData.forEach(group => {
            if (group[header] && group[header] === '当前样品') {
              item.samples.push({
                sampleId: group.sampleId,
                sampleAlias: group.sampleAlias,
                sample: group.sample,
              });
            } else if (group[header] && group[header] !== '当前样品') {
              item.groups.push({
                groupName: group[header],
                color: group[`color_${header.split('_')[1]}`],
                samples: [
                  {
                    sampleId: group.sampleId,
                    sampleAlias: group.sampleAlias,
  ...
  getDataFromUpload = (data, headData) => {
    ...
    dataFromUpload.forEach(item => {
      groupSchemeData.forEach(row => {
        const newItem = item;
        if (item[0] === row.sample || item[0] === row.sampleName) {
          const num = Object.keys(item).length;
          newItem.sample = row.sample;
    ...
    dataFromUpload.forEach(item => {
      ...
      headers.forEach(head => {
        ...
        if (head2color.has(this.getMapKey(item, head))) {
          newItem[this.colorFieldByHead(head)] = head2color.get(
            this.getMapKey(item, head),
          );
          return;
        }
        // 是map中没出现过的,那么就保存起来
        head2color.set(
          this.getMapKey(item, head),
          item[this.colorFieldByHead(head)],
        );
      });
    });
    ...
    this.selectUpdateGroup(null, dataFromUpload, cols);
